"""
================================================================================
AUTOMATED 2-HOP PATH RAG TESTING - v5.9.3
================================================================================

This script tests the 2-Hop Path RAG implementation without needing the full
Flask server. It directly tests the knowledge graph and path finding.

USAGE:
------
python automated_2hop_test.py

OUTPUT:
- Console results with pass/fail status
- JSON report file: test_results_2hop_TIMESTAMP.json
...
class TwoHopPathFinder:
    ...
    def find_2hop_paths(self, entity: str, query: str = "", max_paths: int = 10) -> List[Dict]:
        ...
        if entity_lower in self.relationship_graph:
            for edge1 in self.relationship_graph[entity_lower]:
                target1 = edge1['target']
                path = {
                    'hops': 1,
                    'path_text': f"{entity.upper()} --[{edge1['type']}]--> {target1.upper()}",
                    'relationships': [edge1['type']],
                    'priority': priority_types.index(edge1['type']) if edge1['type'] in priority_types else 99
                }
                paths.append(path)
                
                if target1 in self.relationship_graph:
                    for edge2 in self.relationship_graph[target1]:
                        target2 = edge2['target']
                        if target2 != entity_lower:
                            path2 = {
                                'hops': 2,
                                'path_text': f"{entity.upper()} --[{edge1['type']}]--> {target1.upper()} --[{edge2['type']}]--> {target2.upper()}",
                                'relationships': [edge1['type'], edge2['type']],
                                'priority': min(
                                    priority_types.index(edge1['type']) if edge1['type'] in priority_types else 99,
                                    priority_types.index(edge2['type']) if edge2['type'] in priority_types else 99
                                )
                            }
                            paths.append(path2)
        
        # Sort by: 1) hops first (1-hop before 2-hop), 2) then priority
        # This ensures we get diverse 1-hop relationships before 2-hop expansions
    ...
    def get_context_for_query(self, entities: List[str], query: str) -> Dict:
        ...
        for entity in entities:
            paths = self.find_2hop_paths(entity, query)  # Pass query for intent-based priority
...
def run_all_tests() -> Dict:
    """Run all test cases."""
    print("="*70)
    print("🧪 AUTOMATED 2-HOP PATH RAG TESTING - v5.9.3")
    print("="*70)
    ...
    print("🔧 Initializing 2-Hop Path Finder...")
    ...
    output_file = f"{OUTPUT_DIR}/test_results_2hop_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == '--interactive':
        # Interactive mode
        print("🧪 Interactive 2-Hop Path RAG Testing")
