"""
Entity Extraction Test Runner for SAMM Chapter 7
=================================================

This script tests your Entity Agent against Chapter 7 ground truth entities.
It measures: Precision, Recall, F1 Score, Hallucination Rate, and Confidence.

Chapter 7: Transportation

Usage:
    python run_chapter7_entity_tests.py --url http://172.16.200.12:3000 --pattern all
    python run_chapter7_entity_tests.py --url http://172.16.200.12:3000 --pattern pattern_c7_1_dts
    python run_chapter7_entity_tests.py --url http://172.16.200.12:3000 --start 401 --end 410
"""
...
# Import Chapter 7 test cases
from chapter7_entity_implementation import (
    TEST_CASES_CHAPTER_7,
    CHAPTER_7_GROUND_TRUTH,
    CHAPTER_7_ACRONYM_PAIRS,
    get_all_chapter7_tests,
    get_chapter7_ground_truth_entities
)
...
def calculate_metrics(extracted: List[str], expected: List[str]) -> Dict[str, float]:
    ...
    ground_truth = get_chapter7_ground_truth_entities()
...
def run_pattern_tests(api_url: str, pattern_name: str) -> Dict:
    ...
    if pattern_name not in TEST_CASES_CHAPTER_7:
        print(f"❌ Unknown pattern: {pattern_name}")
        print(f"Available patterns: {list(TEST_CASES_CHAPTER_7.keys())}")
        return None
    
    pattern = TEST_CASES_CHAPTER_7[pattern_name]
    results = []
...
def run_all_tests(api_url: str) -> Dict:
    ...
    for pattern_name in TEST_CASES_CHAPTER_7.keys():
    ...
    if all_summaries:
        ...
        if total_successful > 0:
            ...
            print(f"\n{'='*80}")
            print("OVERALL CHAPTER 7 ENTITY EXTRACTION METRICS")
            print(f"{'='*80}")
            print(f"\n📊 Total Tests: {total_successful}/{total_tests} successful")
            print(f"\n📈 Overall Metrics:")
...
def run_test_range(api_url: str, start_id: int, end_id: int) -> Dict:
    ...
    all_tests = get_all_chapter7_tests()
...
# =============================================================================
# COMBINED TEST RUNNER (Chapter 6 + Chapter 7)
# =============================================================================
def run_combined_tests(api_url: str) -> Dict:
    """Run both Chapter 6 and Chapter 7 tests together"""
    try:
        from chapter6_entity_implementation import (
            TEST_CASES_CHAPTER_6,
            get_chapter6_ground_truth_entities
        )
        has_chapter6 = True
    except ImportError:
        has_chapter6 = False
        print("⚠️  Chapter 6 implementation not found, running Chapter 7 only")
    
    all_results = {}
    all_summaries = []
    
    # Run Chapter 6 tests if available
    if has_chapter6:
        print("\n" + "="*80)
        print("CHAPTER 6: FMS Case Implementation and Execution")
        print("="*80)
        
        for pattern_name in TEST_CASES_CHAPTER_6.keys():
            result = run_pattern_tests_ch6(api_url, pattern_name)
            all_results[f"ch6_{pattern_name}"] = result
            if result and result["summary"]:
                result["summary"]["chapter"] = 6
                all_summaries.append(result["summary"])
    
    # Run Chapter 7 tests
    print("\n" + "="*80)
    print("CHAPTER 7: Transportation")
    print("="*80)
    
    for pattern_name in TEST_CASES_CHAPTER_7.keys():
        result = run_pattern_tests(api_url, pattern_name)
        all_results[f"ch7_{pattern_name}"] = result
        if result and result["summary"]:
            result["summary"]["chapter"] = 7
            all_summaries.append(result["summary"])
    
    # Calculate overall summary
    if all_summaries:
        print(f"\n{'='*80}")
        print("COMBINED CHAPTER 6 + 7 ENTITY EXTRACTION METRICS")
        print(f"{'='*80}")
...
def run_pattern_tests_ch6(api_url: str, pattern_name: str) -> Dict:
    """Run Chapter 6 pattern tests with Ch6 ground truth"""
    from chapter6_entity_implementation import (
        TEST_CASES_CHAPTER_6,
        get_chapter6_ground_truth_entities
    )
    
    if pattern_name not in TEST_CASES_CHAPTER_6:
        return None
    
    pattern = TEST_CASES_CHAPTER_6[pattern_name]
    results = []
    
    print(f"\n  Pattern: {pattern_name} ({len(pattern['tests'])} tests)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Chapter 7 Entity Extraction Test Runner")
    ...
    parser.add_argument("--output", default="chapter7_test_results.json", help="Output file")
    parser.add_argument("--list-patterns", action="store_true", help="List available patterns")
    parser.add_argument("--combined", action="store_true", help="Run both Chapter 6 and Chapter 7 tests")
    
    args = parser.parse_args()
    
    if args.list_patterns:
        print("\n📋 Available Chapter 7 Test Patterns:")
        for name, data in TEST_CASES_CHAPTER_7.items():
            print(f"   {name}: {data['description']} ({len(data['tests'])} tests)")
        exit(0)
    
    print(f"🚀 SAMM Chapter 7 Entity Extraction Test Runner")
    print(f"   Chapter: Transportation")
