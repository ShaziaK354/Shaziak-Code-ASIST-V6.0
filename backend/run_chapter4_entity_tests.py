"""
Entity Extraction Test Runner for SAMM Chapter 4
=================================================

This script tests your Entity Agent against Chapter 4 ground truth entities.
...
Usage:
    python run_chapter4_entity_tests.py --url http://172.16.200.12:3000 --pattern all
    python run_chapter4_entity_tests.py --url http://172.16.200.12:3000 --pattern pattern_c4_1_eligibility
    python run_chapter4_entity_tests.py --url http://172.16.200.12:3000 --start 101 --end 110
"""
...
def calculate_metrics(extracted: List[str], expected: List[str]) -> Dict[str, float]:
    ...
    ground_truth = get_chapter4_ground_truth_entities()
...
def run_pattern_tests(api_url: str, pattern_name: str) -> Dict:
    ...
    if pattern_name not in TEST_CASES_CHAPTER_4:
        print(f"❌ Unknown pattern: {pattern_name}")
        print(f"Available patterns: {list(TEST_CASES_CHAPTER_4.keys())}")
        return None
    
    pattern = TEST_CASES_CHAPTER_4[pattern_name]
    results = []
...
def run_all_tests(api_url: str) -> Dict:
    """Run all Chapter 4 test patterns"""
    
    all_results = {}
    all_summaries = []
    
    for pattern_name in TEST_CASES_CHAPTER_4.keys():
    ...
    if all_summaries:
        print(f"\n{'='*80}")
        print("OVERALL CHAPTER 4 ENTITY EXTRACTION METRICS")
        print(f"{'='*80}")
...
def run_test_range(api_url: str, start_id: int, end_id: int) -> Dict:
    ...
    all_tests = get_all_chapter4_tests()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Chapter 4 Entity Extraction Test Runner")
    ...
    parser.add_argument("--output", default="chapter4_test_results.json", help="Output file")
    parser.add_argument("--list-patterns", action="store_true", help="List available patterns")
    
    args = parser.parse_args()
    
    if args.list_patterns:
        print("\n📋 Available Chapter 4 Test Patterns:")
        for name, data in TEST_CASES_CHAPTER_4.items():
            print(f"   {name}: {data['description']} ({len(data['tests'])} tests)")
        exit(0)
    
    print(f"🚀 SAMM Chapter 4 Entity Extraction Test Runner")
    print(f"   Chapter: Foreign Military Sales (FMS) Program General Information")
