"""
Entity Extraction Test Runner for SAMM Chapter 6
=================================================

This script tests your Entity Agent against Chapter 6 ground truth entities.
It measures: Precision, Recall, F1 Score, Hallucination Rate, and Confidence.

Chapter 6: FMS Case Implementation and Execution

Usage:
    python run_chapter6_entity_tests.py --url http://172.16.200.12:3000 --pattern all
    python run_chapter6_entity_tests.py --url http://172.16.200.12:3000 --pattern pattern_c6_1_implementation
    python run_chapter6_entity_tests.py --url http://172.16.200.12:3000 --start 301 --end 310
"""
...
def calculate_metrics(extracted: List[str], expected: List[str]) -> Dict[str, float]:
    ...
    ground_truth = get_chapter6_ground_truth_entities()
...
def run_pattern_tests(api_url: str, pattern_name: str) -> Dict:
    ...
    if pattern_name not in TEST_CASES_CHAPTER_6:
        ...
        print(f"Available patterns: {list(TEST_CASES_CHAPTER_6.keys())}")
        return None
...
def run_all_tests(api_url: str) -> Dict:
    """Run all Chapter 6 test patterns"""
    ...
    if all_summaries:
        print(f"\n{'='*80}")
        print("OVERALL CHAPTER 6 ENTITY EXTRACTION METRICS")
        print(f"{'='*80}")
...
def run_test_range(api_url: str, start_id: int, end_id: int) -> Dict:
    ...
    all_tests = get_all_chapter6_tests()
...
# =============================================================================
# COMBINED TEST RUNNER (Chapter 5 + Chapter 6)
# =============================================================================
def run_combined_tests(api_url: str) -> Dict:
    """Run both Chapter 5 and Chapter 6 tests together"""
    try:
        ...
        has_chapter5 = False
        print("⚠️  Chapter 5 implementation not found, running Chapter 6 only")
    
    all_results = {}
    all_summaries = []
    
    # Run Chapter 5 tests if available
    if has_chapter5:
        print("\n" + "="*80)
        ...
        for pattern_name in TEST_CASES_CHAPTER_5.keys():
            # Use Chapter 5's calculation function
            result = run_pattern_tests_ch5(api_url, pattern_name)
    ...
    if all_summaries:
        print(f"\n{'='*80}")
        print("COMBINED CHAPTER 5 + 6 ENTITY EXTRACTION METRICS")
        print(f"{'='*80}")
...
def run_pattern_tests_ch5(api_url: str, pattern_name: str) -> Dict:
    """Run Chapter 5 pattern tests with Ch5 ground truth"""
    from chapter5_entity_implementation import (
        TEST_CASES_CHAPTER_5,
        get_chapter5_ground_truth_entities
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Chapter 6 Entity Extraction Test Runner")
    ...
    parser.add_argument("--output", default="chapter6_test_results.json", help="Output file")
    parser.add_argument("--list-patterns", action="store_true", help="List available patterns")
    parser.add_argument("--combined", action="store_true", help="Run both Chapter 5 and Chapter 6 tests")
    
    args = parser.parse_args()
    
    if args.list_patterns:
        print("\n📋 Available Chapter 6 Test Patterns:")
        for name, data in TEST_CASES_CHAPTER_6.items():
            print(f"   {name}: {data['description']} ({len(data['tests'])} tests)")
        exit(0)
    
    print(f"🚀 SAMM Chapter 6 Entity Extraction Test Runner")
    print(f"   Chapter: FMS Case Implementation and Execution")
