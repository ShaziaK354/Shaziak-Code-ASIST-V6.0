"""
SAMM Answering Agent - Automated Metrics Testing Framework v3 FULL
===================================================================
VERSION: 3.0 (compatible with app v5.9.1)

CHANGES from v2:
- Timeout increased: 120s → 200s (reduce timeouts)
- Reduced metric thresholds for realistic CPU-based evaluation:
  * Completeness: 95% → 85%
  * Groundedness: 98% → 80%
  * Citation Accuracy: 95% → 80%

All 15 questions enabled for full testing.
"""
...
# Metric Thresholds - UPDATED for v5.9.1 (realistic CPU-based evaluation)
THRESHOLDS = {
    "completeness": 0.85,       # REDUCED from 0.95
    "groundedness": 0.80,       # REDUCED from 0.98
    "citation_accuracy": 0.80,  # REDUCED from 0.95
    "quality_score": 0.80,
...
class SAMMTestRunner:
    ...
    def call_agent(self, question: str) -> Tuple[str, Dict, float]:
        ...
        try:
            response = requests.post(url, json=payload, timeout=200)  # v3: Increased from 120s to 200s
    ...
    def run_all_tests(self, test_questions: List[Dict] = None, verbose: bool = True) -> List[TestResult]:
        ...
        print(f"\n{'#'*70}")
        print(f"# SAMM ANSWERING AGENT - AUTOMATED METRICS TEST v3 FULL")
    ...
    def export_results_to_excel(self, filepath: str = "samm_metrics_test_results_v3_FULL.xlsx"):
...
def run_tests(debug: bool = False):
    ...
    runner.export_results_to_excel("samm_metrics_test_results_v3_FULL.xlsx")
    return results
