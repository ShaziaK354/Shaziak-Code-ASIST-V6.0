"""
SAMM Answering Agent - Automated Metrics Testing Framework
Tests Answer Quality Metrics based on ASIST Performance Dashboard specifications:
- Completeness (≥95%): Does answer address all parts of the question?
- Groundedness (≥98%): Is answer supported by retrieved documents?
- Citation Accuracy (≥95%): Are citations correct and verifiable?
- Quality Score (≥80%): SAMM standards compliance
- Confidence Score (≥85%): Composite of all metrics
"""
...
API_BASE_URL = "http://127.0.0.1:3000"  # Your agent's URL
TEST_ENDPOINT = "/api/test/query"  # No-auth endpoint for testing
...
class SAMMMetricsEvaluator:
    ...
    def __init__(self, ground_truth: Dict = CHAPTER_1_GROUND_TRUTH):
    ...
    def evaluate_completeness(self, answer: str, test_question: Dict) -> MetricResult:
        ...
        Measurement: Human evaluation (100%, 50%, 0%) → automated approximation
        Target: ≥95%
        """
    ...
    def _get_element_variations(self, element: str) -> List[str]:
        ...
        variations = {
            "definition": ["definition", "defined as", "refers to", "means", "is"],
            "purpose": ["purpose", "objective", "goal", "aim", "intended to"],
            "scope": ["scope", "includes", "encompasses", "comprises", "covers"],
    ...
    def evaluate_groundedness(self, answer: str, test_question: Dict) -> MetricResult:
        """
        Evaluates if answer is supported by retrieved documents (no hallucination).
        Measurement: Every claim must have source in retrieved context
        Target: ≥98%
        """
        ...
        # Check how many key content items are reflected in the answer
        grounded_claims = 0
        total_checkable = len(key_content) + len(required_terms)
        
        for content in key_content:
            # Check if key phrases from ground truth appear in answer
        ...
        # Check for potential hallucinations (claims not in ground truth)
    ...
    def evaluate_citation_accuracy(self, answer: str, test_question: Dict) -> MetricResult:
        ...
        Measurement: Citation points to correct chapter/section that supports claim
        Target: ≥95%
        """
        ...
        if not expected_citations:
            # No expected citations, check if any citations are present
        ...
        # Check accuracy of found citations
        correct_citations = []
        incorrect_citations = []
        ...
        for citation in found_citations:
            ...
            if is_correct:
                ...
                # Check if citation is a valid SAMM section (not wrong, just extra)
                if re.match(r'C\d+\.\d+', citation):
                    correct_citations.append(citation)  # Valid section format
                else:
        ...
        # Bonus for having all expected citations
    ...
    def evaluate_quality_score(self, answer: str, test_question: Dict) -> MetricResult:
        ...
        if guidelines["min"] <= answer_len <= guidelines["max"]:
            ...
            scores["length"] = QUALITY_WEIGHTS["length"] * 0.7  # Too long
        ...
        if "." in answer and len(answer.split(".")) > 1:
            structure_score += 0.4  # Complete sentences
        if any(term in answer for term in ["Security Cooperation", "Security Assistance"]):
            structure_score += 0.3  # Uses proper terminology
        if citations:
            structure_score += 0.3  # Has citations
    ...
    def calculate_confidence_score(self, completeness: MetricResult, 
        ...
        Formula: Completeness (30%) + Groundedness (30%) + Citation (20%) + Quality (20%)
        Target: ≥85%
        """
        weights = {
            "completeness": 0.30,
...
class SAMMTestRunner:
    ...
    def __init__(self, api_url: str = API_BASE_URL, endpoint: str = TEST_ENDPOINT):
    ...
    def call_agent(self, question: str) -> Tuple[str, Dict, float]:
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                # Extract answer from response (adjust based on your API structure)
    ...
    def run_all_tests(self, test_questions: List[Dict] = None, verbose: bool = True) -> List[TestResult]:
        ...
        print(f"\n{'#'*70}")
        print(f"# SAMM ANSWERING AGENT - AUTOMATED METRICS TEST")
    ...
    def export_results_to_excel(self, filepath: str = "test_results.xlsx"):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--quick":
            # Quick test with first 3 questions
            runner = SAMMTestRunner()
            runner.run_all_tests(CHAPTER_1_TEST_QUESTIONS[:3])
        elif sys.argv[1] == "--offline":
            # Offline mode - just print test structure
            print("Test Questions:")
            for q in CHAPTER_1_TEST_QUESTIONS:
